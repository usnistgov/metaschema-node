...
describe('XmlChoiceInstance', () => {
    it('should load from XML', async () => {
        const metaschema = await XmlMetaschema.load(
            ...
            <METASCHEMA xmlns="http://csrc.nist.gov/ns/oscal/metaschema/1.0">
                ...
                <define-field name="title" as-type="markup-line">
                    ...
                    <description>A test field representing a 'title' type</description>
                </define-field>

                <define-field name="id" as-type="token">
                    <formal-name>Identifier</formal-name>
                    <description>A test field representing an 'id' type</description>
                </define-field>

                <define-assembly name="testAssembly">
                    <formal-name>Test Assembly</formal-name>
                    <description>A test assembly containing a choice</description>
                    <model>
                        <choice>
                            <field ref="title" max-occurs="1"/>
                            <field ref="id" max-occurs="1"/>
                        </choice>
                    </model>
                </define-assembly>
            </METASCHEMA>`,
        );
        // grab out the choice instance
        const choice = metaschema.getScopedAssemblyDefinitionByName('testAssembly')?.getChoiceInstances()[0];
        if (choice === undefined) {
        ...
        expect(choice.getModelType()).toBe(ModelType.CHOICE);
    });
});
